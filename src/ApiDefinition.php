...
use Raml\FileLoader\fileLoaderTrait;
use Raml\Example\exampleIteratorTrait;
...
class ApiDefinition implements ArrayInstantiationInterface {
  ...
  public static function createFromArray($title, array $data = []) {
    ...
    $apiDefinition->setRequestDefaultsClassName($data['configuration']->getRequestDefaultsClassName());
    $apiDefinition->setExampleIteratorClassName($data['configuration']->getExampleIteratorClassName());
    $apiDefinition->setFileLoaderClassName($data['configuration']->getFileLoaderClassName());
  ...
  /**
   * Returns all the resources as a URI, essentially documenting the entire API
   * Definition. This will output, by default, an array that looks like:
   *
  ...
  /**
   * Add an additional security scheme to the list of schemes the whole API is
   * secured by
   *
   * @param SecurityScheme $securityScheme
   */
  ...
  /**
   * Recursive function that generates a flat array of the entire API
   * Definition
   *
   * GET /songs => [api.example.org, /songs, GET, [https], Raml\Method]
   * GET /songs/{songId} => [api.example.org, /songs/{songId}, GET, [https],
   * Raml\Method]
   *
