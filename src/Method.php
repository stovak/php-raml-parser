...
class Method implements ArrayInstantiationInterface {
   ...
   */
  public function __construct($type, ApiDefinition $apiDefinition, $uri) {
    $this->type = strtoupper($type);
    $this->uri = $uri;
    foreach ($apiDefinition->getProtocols() as $protocol) {
      $this->addProtocol($protocol);
    }
    $this->setRequestDefaultsClassName($apiDefinition->getRequestDefaultsClassName());
  }
  ...
  public static function createFromArray($method, array $data = [], ApiDefinition $apiDefinition = NULL) {
    ...
    $method = new static($method, $apiDefinition, $data['uri']);
    ...
    if (isset($data['examples'])) {
      $method->setExamples($data['examples'], $apiDefinition);
    }
  ...
  public function addProtocol($protocol) {
    if (!in_array($protocol, [
      ApiDefinition::PROTOCOL_HTTP,
      ApiDefinition::PROTOCOL_HTTPS
    ])
    ) {
   ...
   * @param bool           $merge Set to true to merge the security scheme data
   *                              with the method, or false to not merge it.
   */
